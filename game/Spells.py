...
class SummonStormDrakeSpell(Spell):
	...
	def on_init(self):
		...
		self.upgrades['ghost_drake'] = (1, 3, "Ghost Drake", "给予[Storm Drake:unit][ghostly]词条", [Tags.Dark])
...
class SummonVoidDrakeSpell(Spell):
	...
	def on_init(self):
		...
		self.upgrades['essence_drake'] = (1, 4, "Essence Drake", "当[Void Drake:unit]杀死敌人时，一个随机临时盟友转为永久")
...
class SummonIceDrakeSpell(Spell):
	...
	def on_init(self):
		...
		self.upgrades['broodlings'] = (1, 4, "Broodlings", "[Ice Drake:unit]可以召唤两只[Ice Lizard:unit]，冷却时间 [9:cooldown]")
		self.upgrades['dragon_mage'] = (1, 5, "Dragon Mage", "[Ice Drake:unit]可以施放你的[Death Chill:spell]，冷却时间 [8:cooldown]", [Tags.Dark])
		self.upgrades['dracolich'] = (1, 6, "Dracolich", "召唤[Dracolich:unit]而不是[Ice Drake:unit].", [Tags.Dark])
...
class ChainLightningSpell(Spell):
	...
	def on_init(self):
		...
		self.upgrades['chain_fireball'] = (1, 7, "Chain Fireball", "对[Chain Lightning:spell]的每个连锁目标施放[Fireball:spell]", [Tags.Fire], [Tags.Fire])
		self.upgrades['weathercraft'] = (1, 3, "Cloud Conductance", "[Chain Lightning:spell]可以通过雨云、暴风雪和雷暴连锁", [Tags.Nature])
		self.upgrades['shield'] = (1, 4, "Lightning Shield", "[Chain Lightning:spell]可以连锁友方单位\n友方单位不会受伤而是获得 [1:shields]，最多 3 点")

	def get_description(self):
		return ("发射闪电链，对你和目标的连线造成 [{damage}:lightning]\n"
				"闪电链会连锁到[{cascade_range}:cascade_range]内的新目标，对新旧目标的连线造成同样的伤害\n"
...
class DeathBolt(Spell):
	...
	def on_init(self):
		...
		self.upgrades['soulbattery'] = (1, 3, "Soul Battery", "[Death Bolt:spell]击杀目标时给你的[dark]法术和能力提供 [1:damage]，持续到本关结束")
		self.upgrades['winter'] = (1, 3, "Winter Bolt", "[Death Bolt:spell]额外造成一份[ice]伤害", [Tags.Ice], [Tags.Ice])
		self.upgrades['chaos'] = (1, 5, "Chaos Skeletons", "[Skeleton:unit]拥有[chaos]词条，获得抗性，施放混沌球和死亡时召唤和最大生命值成比例数量的小鬼的能力", [Tags.Chaos])
...
class TouchOfDeath(Spell):
	...
	def on_init(self):
		...
		self.upgrades['fire_damage'] = (1, 3, "Flametouch", "[Touch of Death:spell]额外造成一份[fire]伤害", [Tags.Fire], [Tags.Fire])
		self.upgrades['vampire'] = (1, 3, 'Touch of the Vampire', '[Touch of Death:spell]击杀[living]单位时，将其复活为[Vampire:unit]', [Tags.Conjuration])
		self.upgrades['hand_of_death'] = (1, 3, 'Hand of Death', '[Touch of Death:spell]额外伤害最多 [4:num_targets]相邻单位')
		self.upgrades['final_touch'] = (1, 5, 'Final Touch', "[Touch of Death:spell]造成和目标最大生命值相等的额外伤害")
...
class UnderworldPortal(Spell):
	...
	def on_init(self):
		...
		self.upgrades['summon_rockworms'] = (1, 3, "Tremorsensitivity", "在两端分别召唤两只[Rock Worm:unit]", [Tags.Conjuration])
...
class SummonEarthElemental(Spell):
	...
	def on_init(self):
		...
		self.upgrades['earthquake_totem'] = (1, 4, "Earthquake Totem", "[Earth Elemental:unit]可以施放你的[Earthquake:spell] 冷却时间 [3:cooldown]")
		self.upgrades['stinging_totem'] = (1, 3, "Stinging Totem", "[Earth Elemental:unit]可以施放你的[Poison Sting:spell]")
		self.upgrades['holy_totem'] = (1, 6, "Holy Totem", "[Earth Elemental:unit]可以施放你的[Heavenly Blast:spell] 冷却时间 [2:cooldown]", [Tags.Holy])
...
class CallSpirits(Spell):
	...
	def on_init(self):
		...
		self.upgrades['king'] = (1, 5, "Ghost King", "在选定地块召唤[Ghost King:unit]")
		self.upgrades['mass'] = (1, 4, "Ghost Mass", "在选定地块召唤[Ghostly Mass:unit]")
...
class DeathGazeSpell(Spell):
	...
	def on_init(self):
		...
		self.upgrades['toxic_gaze'] = (1, 3, "Toxic Gaze", "[Vampiric Gaze:spell]额外造成一份[poison]伤害", [Tags.Nature])
...
class InvokeSavagerySpell(Spell):
	...
	def on_init(self):
		...
		self.upgrades['poison'] = (10, 1, "Venomous Bite", "施加 [10:duration][poisoned]")
		self.upgrades['leap_attack'] = (1, 4, "Savage Leap", "单位改为跃击[3:radius]内的敌人")
		self.upgrades['recharge_wolf'] = (1, 3, "Scavengers", "每击杀三个敌人给予一点[Wolf:spell]充能")
...
class MagicMissile(Spell):
	...
	def on_init(self):
		...
		self.upgrades['shield_burn'] = (1, 2, "Shield Burn", "[Magic Missile:spell]造成伤害之前移除所有护盾")
		self.upgrades['disruption'] = (1, 2, "Disruption Bolt", "[Magic Missile:spell]对[arcane]单位造成[dark]和[holy]伤害，而不是[arcane]伤害", None, [Tags.Dark, Tags.Holy])
...
class MindDevour(Spell):
	...
	def get_description(self):
		return ("对敌方单位造成 [{damage}:arcane] 和 [{damage}:dark]\n"
				"之后，如果目标生命值少于一半，额外造成一次\n"
				"击杀目标时将其复活为僵尸\n"
				"只能对[living]、[demon]和[arcane]单位施放").format(**self.fmt_dict())
...
class DeathShock(Spell):
	...
	def on_init(self):
		...
		self.upgrades['fire'] = (1, 5, 'Fire Shock', "额外造成一份[fire]伤害", [Tags.Fire], [Tags.Fire])
		self.upgrades['shield_burn'] = (3, 4, 'Shield Burn', "造成伤害之前移除 [3:shields]") # weird, replace?
		self.upgrades['corpse'] = (1, 4, 'Corpse Construct', "如果[Death Shock:spell]击杀了至少一个敌人，召唤一个闪电僵尸，使用所有击杀敌人总生命值的一半", [Tags.Conjuration]) # ……闪电僵尸
...
class WordOfUndeath(Spell):
	...
	def on_init(self):
		...
		self.upgrades['hatred_of_life'] = (1, 3, "Hatred of Life", "对你以外所有[living]单位造成 [36:poison]")
...
class MassCalcification(Spell):
	...
	def on_init(self):
		...
		self.upgrades['fae_bones'] = (1, 4, "Fae Bones", "赋予[arcane]单位转化的[Bone Shambler:unit][fae]词条", [Tags.Arcane])
		self.upgrades['burning_bones'] = (1, 4, "Burning Bones", "赋予[fire]单位转化的[Bone Shambler:unit][burning]词条", [Tags.Fire])
...
class WormOffering(Spell):
	...
	def on_init(self):
		...
		# Ultimate Upgrades
		self.upgrades['toxic'] = (1, 4, "Toxogenesis", "改为召唤[剧毒蠕虫球:unit]，拥有[poison]光环。")
		self.upgrades['iron'] = (1, 5, "Mechanogenesis", "改为召唤[铁质蠕虫球:unit]，拥有多种抗性。", [Tags.Metallic])
		self.upgrades['ghostly'] = (1, 5, "Ectogenesis", "改为召唤[幽灵蠕虫球:unit]，拥有多种抗性，[undead]，拥有传送能力。", [Tags.Dark]))
...
class HagSummon(Spell):
	...
	def on_init(self):
		...
		self.upgrades["nightmare"] = (1, 4, "Nightmare Hag", "老鬼婆可施放你的梦魇光环法术。", [Tags.Arcane])
		self.upgrades["bone_lady"] = (1, 6, "Bone Queen", "老鬼婆可召唤 [2:num_summons]小型骸骨跛行者。")
...
class BloodGolemVampirism(Buff):
	...
	def on_init(self):
		...
		self.description = "造成的所有伤害的一半转化为治疗，分配给所有[living]盟友。"
		self.global_triggers[EventOnDamaged] = self.on_damage
...
class BloodGolemSpell(Spell):
	...
	def on_init(self):
		...
		self.upgrades['pain_aura'] = (1, 3, "Pain Aura", "鲜血魔像获得一个[5:radius]的伤害光环，每回合随机造成[2 点:damage][poison]或[fire]伤害。", [Tags.Dark])
...
class BloodTapSpell(Spell):
	...
	def on_init(self):
		...
		self.upgrades['bond'] = (1, 2, "Blood Bond", "生命吸取额外治疗[2:num_targets]随机[living]盟友。")
		self.upgrades['chain'] = (1, 5, "Life Funnel", "生命吸取额外从目标至多[3:radius]内的[2:num_targets]随机敌方单位处吸取生命。")
		self.upgrades['requires_los'] = (-1, 1, "Blindcasting", ignore_los_upgrade)
...
class RitualOfRevelation(Spell):
	...
	def on_init(self):
		...
		self.upgrades['blasphemy'] = (1, 5, "Blasphemy", "赋予[False Prophet:unit][lich]词条。", [Tags.Dark])
		self.upgrades['arcane_revelation'] = (1, 8, "Arcane Revelation", "仪式每回合也会对 [3:num_targets]敌人施放[Blazerip:spell]。", [Tags.Arcane])
...
class DevourFlesh(Spell):
	...
	def on_init(self):
		...
		self.upgrades['charred_flesh'] = (1, 4, "Charred Flesh", "被杀死的活物单位重生为带燃烧词条的骷髅。", [Tags.Dark, Tags.Fire]) #Added Living requirement due to raise_skeleton(), can change to normal summon to remove req
...
class Bloodshift(Spell):
	...
	def on_init(self):
		...
		self.upgrades['toxicity'] = (1, 4, "Toxic Shift", "鲜血转移也会造成[poison]伤害，这部分伤害不会提供治疗。", None, [Tags.Poison])
...
class FleshFiendSpell(Spell):
	...
	def on_init(self):
		...
		self.upgrades['char_fiend'] = (1, 5, "Barbeque Fiends", "召唤带燃烧词条的血肉集群，而非普通版本。", [Tags.Fire])
...
class HelgateSpell(Spell):
	...
	def on_init(self):
		...
		self.upgrades['fae_aelves'] = (1, 6, "Fae Aelves", "刷怪笼生成仙灵词条的精灵而非普通精灵。", [Tags.Arcane])
...
class StampedeFormSpell(Spell):
	...
	def on_init(self):
		...
		self.upgrades['fae'] = (1, 6, "Fae Stampede", "赋予象群[fae]词条。[Stampede Form:spell]提供 [50:r_arcane]，施放奥术法术不会结束大象形态。", [Tags.Arcane])
		self.upgrades['burning'] = (1, 7, "Burning Stampede", "赋予象群[fire]词条。[Stampede Form:spell]提供 [50:r_fire]，施放火焰法术不会结束大象形态。", [Tags.Fire])
		self.upgrades['metallic'] = (1, 7, "Metal Stampede", "赋予象群[metallic]词条。[Stampede Form:spell]提供 [25:r_fire]和 [50:r_lightning]，施放金属法术不会结束大象形态。", [Tags.Metallic])
...
class ChannelMalevolence(Spell):
	...
	def on_init(self):
		...
		self.upgrades['toxic'] = (1, 3, "Toxic Hatred", "箭也会对敌人造成 [poison]，持续 [13_turns:duration]", [Tags.Nature])
	...
	def get_description(self):
		return ("每回合对[{num_targets}:num_targets]随机敌人发射能量箭，交替对路径上所有单位造成[{damage}:fire]或[{damage}:dark]。\n"
				+ loc.clauses['channel'] % "[{max_channel}:duration]"
				+ "最后，每个剩余敌人对施法者造成 [2 点:damage][fire]或[dark]伤害").format(**self.fmt_dict())
