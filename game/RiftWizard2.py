...
UNVICTORIED_TARGET = TooltipExamineTarget("只展示尚未获得胜利的选项")
...
class PyGameView(object):
	...
	def draw_char_sheet(self):
		...
		if equipment:
			self.draw_string("装备", self.middle_menu_display, cur_x, cur_y)
	...
	def open_buy_prompt(self, item):
		...
		if isinstance(item, Equipment):
			...
			if isinstance(self.game.cur_level.cur_shop, AmnesiaShop) and isinstance(item, Spell):
				self.confirm_text = "遗忘 %s?" % self.chosen_purchase.name
	...
	def draw_shop(self):
		...
		if self.shop_type in [SHOP_TYPE_UPGRADES, SHOP_TYPE_SPELLS]:
			...
			self.draw_string("未拥有", self.middle_menu_display, cur_x, cur_y, color, mouse_content=UNPURCHASED_TARGET, content_width=tag_width)

			color = (255, 255, 255) if self.filter_unvictoried else (150, 150, 150)
			cur_y += self.linesize
			self.draw_string("未得胜", self.middle_menu_display, cur_x, cur_y, color, mouse_content=UNVICTORIED_TARGET, content_width=tag_width)
	...
	def draw_character(self):
		...
		if self.game.p1.equipment or self.game.p1.trinkets:
			...
			for equipment in equipments:
				...
				self.make_content_rect(self.character_display, abs_rect, equipment)
		...
		if getattr(self.game, 'rift_rerolls', 0): # blocks a crash that came from trying to load a save that resulted in a crash
			if self.game.rift_rerolls == 1:
				...
				self.draw_string("重骰裂隙 (R) (%d)" % self.game.rift_rerolls, self.character_display, cur_x, cur_y, mouse_content=REROLL_PORTALS_TARGET)
		cur_y += linesize
	...
	def draw_examine_upgrade(self):
		...
		for spell, bonuses in self.examine_target.spell_bonuses_pct.items():
			...
			for attr, val in useful_bonuses:
				...
				if attr in tooltip_colors:
					fmt = "%s 获得 [%s%%_点%s:%s]" % (spell_name, val, attr, attr)
				else:
		...
		for spell, bonuses in self.examine_target.spell_bonuses.items():
			...
			for attr, val in useful_bonuses:
				spell_name = loc.spells.get(spell_ex.name, spell_ex.name)
				if attr in tooltip_colors:
					fmt = "%s 获得 [%s_%s:%s]" % (spell_name, val, attr, attr)
				else:
					fmt = "%s 获得 %d %s" % (spell_name, val, format_attr(attr))
				lines = self.draw_wrapped_string(fmt, self.examine_display, cur_x, cur_y, width=width)
				cur_y += (lines+1) * self.linesize
	...
	def draw_examine_spell(self):
		...
		if spell.hp_cost:
			self.draw_string("生命消耗: %d" % spell.get_stat('hp_cost'), self.examine_display, cur_x, cur_y)
			cur_y += self.linesize
		...
		for spell in unit.spells:
			...
			if hasattr(spell, 'damage'):
				if hasattr(spell, 'damage_type') and isinstance(spell.damage_type, Tag):
					...
					is_random = getattr(spell, 'damage_type_random', False)
					connector = '或' if is_random else '和'
					damage = spell.get_stat('damage')
					damage_type = (['[%s]' % t.name for t in spell.damage_type])
                    damage_type = connector.join(damage_type)
					fmt = ' %d 点%s伤害' % (damage, (' %s ' % connector).join(damage_type))
				else:
			...
			if hasattr(spell, 'radius') and spell.get_stat('radius') > 0:
				fmt = ' 半径 %d 格' % spell.get_stat('radius')
			...
			if spell.get_stat('cool_down') > 0:
				...
				if not rem_cd:
					fmt = ' 冷却时间 %d 回合' % spell.get_stat('cool_down')
				else:
					fmt = ' 冷却时间 %d 回合 (%d 回合后可用)' % (spell.get_stat('cool_down'), rem_cd)
				self.draw_string(fmt, self.examine_display, cur_x, cur_y)
				cur_y += linesize
				hasattrs = True
