...
class PyGameView(object):
  ...
  def draw_char_sheet(self):
    ...
    col_width = (self.middle_menu_display.get_width() // 2 - 2 * self.border_margin) // 3 - 20
    ...
    for spell in self.game.p1.spells:
      ...
      exc_class_applied = False
      for u in spell.spell_upgrades:
        if u.applied and not u.exc_class is None:
          exc_class_applied = True
          break
      for u in spell.spell_upgrades:
        if (exc_class_applied and not u.exc_class is None):
          continue
  ...
  def draw_confirm(self):
    ...
    cur_x = (self.middle_menu_display.get_width()) * 3 // 4 - self.space_width * 2
  ...
  def draw_key_rebind(self):
    ...
    self.draw_string("键位 1", self.screen, col_xs[1], cur_y)
    self.draw_string("键位 2", self.screen, col_xs[2], cur_y)
    ...
    self.draw_string("重置", self.screen, cur_x, cur_y, mouse_content=KEY_BIND_OPTION_RESET if not self.rebinding else None)
    cur_y += self.linesize
  ...
  def draw_shop(self):
    ...
    if self.shop_type == SHOP_TYPE_SPELLS:
      ...
      self.draw_string("SP", self.middle_menu_display, level_x - self.space_width, cur_y, COLOR_XP)
    ...
    if self.shop_type == SHOP_TYPE_UPGRADES:
      ...
      self.draw_string("SP", self.middle_menu_display,
                       level_x - self.space_width, cur_y, COLOR_XP)
      self.draw_string("类别", self.middle_menu_display, cur_x + tag_offset, cur_y)
    if self.shop_type == SHOP_TYPE_SPELL_UPGRADES:
      self.draw_string("升级%s" % self.shop_upgrade_spell.show_name, self.middle_menu_display, cur_x, cur_y)
    if self.shop_type == SHOP_TYPE_SHOP: # Shrines.py L2070
      self.draw_string(self.get_display_level().cur_shop.show_name, self.middle_menu_display, 0, cur_y, content_width=self.middle_menu_display.get_width(), center=True)
    if self.shop_type == SHOP_TYPE_BESTIARY:
    ...
    if not shoptions:
      if self.shop_type == SHOP_TYPE_SHOP:
        self.draw_string("没有神龛可以增强的法术。", self.middle_menu_display, 0, cur_y, content_width=self.middle_menu_display.get_width(), center=True)
      elif self.shop_type in [SHOP_TYPE_SPELLS, SHOP_TYPE_SPELLS]:
        self.draw_string("没有符合条件的法术", self.middle_menu_display, cur_x, cur_y, HIGHLIGHT_COLOR)
  ...
  def draw_character(self):
    ...
    for spell in self.game.p1.spells:
      ...
      # 针对字体自定义
      self.draw_spell_icon(spell, self.character_display, cur_x + self.space_width * 3, cur_y)

      cur_y += linesize
      index += 1

    cur_y = max(_y_end, cur_y)
    cur_x = self.border_margin
    cur_y += linesize
    ...
    if status_effects:
      ...
      for buff_name, (buff, stacks, duration, color) in counts.items():
        ...
        fmt = dict_attr.names.get(buff_name, buff_name)
  ...
  def draw_examine_upgrade(self):
    ...
    for tag, bonuses in self.examine_target.tag_bonuses.items():
      for attr, val in bonuses.items():
        tag_name = dict_school.color.get(tag.name, tag.name)
        attr_name = dict_attr.names.get(attr, attr)
        quantifier = dict_attr.quantifiers.get(attr, '')
        fmt = "%s法术和能力获得 [%s_%s%s:%s]。" % (tag_name, val, quantifier, attr_name, attr)
  ...
  def draw_examine_portal(self):
    ...
    for image, name, color in images:
      ...
      _name = dict_monsters.getLocale(name)
  ...
  def draw_examine_unit(self):
    ...
    _name = dict_monsters.getLocale(unit.name)
    ...
    for tag in unit.tags:
      _name = dict_attr.names.get(tag.name, tag.name)
      self.draw_string(_name, self.examine_display, cur_x, cur_y, (tag.color.r, tag.color.g, tag.color.b))
      cur_y += linesize
    ...
    for spell in unit.spells:
      ...
      if hasattr(spell, 'damage'):
        if hasattr(spell, 'damage_type') and isinstance(spell.damage_type, Tag):
          _name = dict_attr.names.get(spell.damage_type.name, spell.damage_type.name)
          fmt = ' [%d_点%s伤害:%s]' % (spell.get_stat('damage'), _name, spell.damage_type.name)
        elif hasattr(spell, 'damage_type') and isinstance(spell.damage_type, list):
          fmt = ' [%d:damage] 点%s伤害' % (spell.damage, '或'.join([dict_attr.color.get(t.name, t.name) for t in spell.damage_type]))
        else:
          fmt = ' [%d_点伤害:damage]' % spell.get_stat('damage')
        lines = self.draw_wrapped_string(fmt, self.examine_display, cur_x, cur_y, self.examine_display.get_width() - 2 * border_margin)
        cur_y += lines * linesize
        hasattrs = True
      if spell.range > 1.5:
  ...
  def draw_pick_mode(self):
    ...
    rect_w = self.space_width * 12
    cur_x = self.screen.get_width() // 2 - self.space_width * 6
  ...
  def draw_options_menu(self):
    ...
    cur_x = self.screen.get_width() // 2 - self.space_width * 8
  ...
  def draw_combat_log(self):
    ...
    self.draw_string("第 %d 关" % self.combat_log_level, self.middle_menu_display, cur_x, cur_y)
    cur_y += self.linesize
    self.draw_string("第 %d 回合" % self.combat_log_turn, self.middle_menu_display, cur_x, cur_y)
    cur_y += self.linesize
    cur_y += self.linesize
